...
vx_node vxComplementaryNode(vx_graph graph, vx_image input, vx_image output)
{
    ...
    {
        vx_kernel kernel = vxGetKernelByName(context, "it.unibo.retina.complementary");
        if (kernel)
        {
...
vx_node vxGaussian5x5Node(vx_graph graph, vx_image input, vx_scalar sigma, vx_image output)
{
    ...
    if (status == VX_SUCCESS)
    {
        vx_kernel kernel = vxGetKernelByName(context, "it.unibo.retina.gaussian5x5");
        if (kernel)
        {
            node = vxCreateGenericNode(graph, kernel);
            if (node)
...
vx_node vxThresholdToZeroNode(vx_graph graph, vx_image input, vx_scalar th, vx_image output)
{
    ...
    if (status == VX_SUCCESS)
    {
        vx_kernel kernel = vxGetKernelByName(context, "it.unibo.retina.threshold_to_zero");
        if (kernel)
        {
            node = vxCreateGenericNode(graph, kernel);
            if (node)
...
vx_status vxuSigmoid(vx_image input, vx_scalar mean, vx_scalar std, vx_scalar slope, vx_scalar minRange, vx_scalar maxRange, vx_image output)
{
    ...
    if (graph)
    {
        vx_node node = vxSigmoidNode(graph, input, mean, std, slope, minRange, maxRange, output);
        if (node)
        {
            status = vxVerifyGraph(graph);
            if (status == VX_SUCCESS)
...
vx_node vxIntegralWindowNode(vx_graph graph, vx_image input, vx_scalar winSize, vx_image output)
{
    vx_uint32 i;
    vx_node node = 0;
    vx_context context = vxGetContext(graph);
    ...
    if (status == VX_SUCCESS)
    {
        vx_kernel kernel = vxGetKernelByName(context, "it.unibo.retina.integral_window");
        if (kernel)
        {
            node = vxCreateGenericNode(graph, kernel);
            if (node)
...
vx_status vxuIntegralWindow(vx_image input, vx_scalar winSize, vx_image output)
{
    vx_context context = vxGetContext(input);
    vx_status status = VX_FAILURE;
    vx_graph graph = vxCreateGraph(context);
...
vx_status vxuDisparity(vx_image input, vx_image minVals, vx_image current, vx_scalar level, vx_image minOut, vx_image output)
{
    ...
    if (graph)
    {
        vx_node node = vxDisparityNode(graph, input, minVals, current, level, minOut, output);
        if (node)
        {
            status = vxVerifyGraph(graph);
            if (status == VX_SUCCESS)
