...
int main(int argc, char *argv[])
{
    ...
    if (context)
    {
        ...
        vx_pyramid pyramid_new = vxCreatePyramid(context, 4, VX_SCALE_PYRAMID_HALF, width, height, VX_DF_IMAGE_U8);
        vx_pyramid pyramid_old = vxCreatePyramid(context, 4, VX_SCALE_PYRAMID_HALF, width, height, VX_DF_IMAGE_U8);
        ...
        if (graph)
        {
            // Nodes
            vx_node nodes[] = {
              vxOpticalFlowPyrLKNode(graph, pyramid_old, pyramid_new,
                                            old_features, old_features, new_features,
                                            criteria, epsilon_s, num_iterations_s,
