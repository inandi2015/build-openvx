...
vx_status ComplementaryOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
...
vx_status ComplementaryKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (num == 2)
    {
        ...
        status |= vxCommitImagePatch(input, 0, 0, &addr_in, in); // don't write back into the input
...
vx_status NormalizeInputValidator(vx_node node, vx_uint32 index)
{
    ...
    if (index == 0) /* input image */
    {
        ...
        {
            vx_image image = 0;
            vxQueryParameter(param, VX_PARAMETER_ATTRIBUTE_REF, &image, sizeof(vx_image));
            if (image)
            {
                vx_fourcc fourcc = 0;
        ...
        if (param)
        {
            ...
            if (input)
            {
                ...
                if(scalar)
                {
                    ...
                    vxQueryScalar(scalar, VX_SCALAR_ATTRIBUTE_TYPE, &typeScalar, sizeof(typeScalar));
                    switch (format)
                    {
                        case FOURCC_U8:
                            type = VX_TYPE_UINT8;
...
vx_status NormalizeOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
...
vx_status NormalizeKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (num == 4)
    {
...
vx_status Gaussian5x5OutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
...
vx_status Gaussian5x5Kernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    ...
    if (num == 3)
    {
        ...
        if(sigma == 0.0f) sigma = 1.1f; // OpenCV formula: ((n-1)*0.5 - 1)*0.3 + 0.8
        ...
        if (borders.mode == VX_BORDER_MODE_UNDEFINED)
        {
            ...
            for (y = 2; (y < (src_addr.dim_y - 2)) && (status == VX_SUCCESS); y++)
            {
                ...
                {
                    void *srcp[25] = {vxFormatImagePatchAddress2d(src_base, x-2, y-2, &src_addr),
...
vx_status ThresholdToZeroOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 2) /* output image */
    {
...
vx_status ThresholdToZeroKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_FAILURE;
    if (num == 3)
    {
        ...
        for (y = 0; y < src_addr.dim_y; y++)
        {
            for (x = 0; x < src_addr.dim_x; x++)
            {
                ...
                {
                    vx_float32 *src_ptr = vxFormatImagePatchAddress2d(src_base, x, y, &src_addr);
                    vx_float32 *dst_ptr = vxFormatImagePatchAddress2d(dst_base, x, y, &dst_addr);
                    if (*src_ptr > value32)
                        *dst_ptr = *src_ptr;
                    else
                        *dst_ptr = 0;
...
vx_status TruncateKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_FAILURE;
    if (num == 3)
    {
...
vx_status AddScalarOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 3) /* output image */
    {
...
vx_status AddScalarKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_FAILURE;
    if (num == 4)
    {
...
vx_status DivideOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 2) /* output image */
    {
...
vx_status SigmoidInputValidator(vx_node node, vx_uint32 index)
{
    ...
    else if (index >= 1 && index <=5) /* mean, std, slope, minRange, maxRange (float scalars) */
    {
        vx_parameter param = vxGetParameterByIndex(node, index);
        if (param)
        {
...
vx_status SigmoidOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
        ...
        for (y = 0; y < src_addr.dim_y; y++)
        {
            for (x = 0; x < src_addr.dim_x; x++)
            {
                if (format == FOURCC_U8)
                {
                    ...
                    vx_float32 result = halfRangeMin + (halfRange * tanh((vx_float32) (alpha * ((vx_float32)(*src_ptr) - mean))));
                ...
                else if (format == FOURCC_F32)
                {
                    ...
                    vx_float32 result = halfRangeMin + (halfRange * tanh((vx_float32) (alpha * (*src_ptr - mean))));
...
vx_status MeanOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 1)
    {
    ...
    if (num == 2)
    {
        ...
        for (y = 0; y < addrs.dim_y; y++)
        {
            for (x = 0; x < addrs.dim_x; x++)
            {
                ...
                {
                    vx_float32 *pixel = vxFormatImagePatchAddress2d(base_ptr, x, y, &addrs);
                    sum += *pixel;
                }
#endif
            }
...
vx_status StdDevOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 2)
    {
...
vx_status Max3x3OutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 1)  /* Output image */
    {
    ...
    if (num == 2)
    {
        ...
        if (borders.mode == VX_BORDER_MODE_UNDEFINED)
        {
            ...
            for (y = 1; (y < (src_addr.dim_y - 1)) && (status == VX_SUCCESS); y++)
            {
                ...
                {
                    void *srcp[9] = { vxFormatImagePatchAddress2d(src_base, x-1, y-1, &src_addr),
                                      vxFormatImagePatchAddress2d(src_base, x+0, y-1, &src_addr),
                                      vxFormatImagePatchAddress2d(src_base, x+1, y-1, &src_addr),
                                      vxFormatImagePatchAddress2d(src_base, x-1, y+0, &src_addr),
                                      vxFormatImagePatchAddress2d(src_base, x+0, y+0, &src_addr),
    ...
    if (num == 2)
    {
        ...
        if (borders.mode == VX_BORDER_MODE_UNDEFINED)
        {
            ...
            for (y = 3; (y < (src_addr.dim_y - 3)) && (status == VX_SUCCESS); y++)
            {
                ...
                {
                    void *srcp[49] = {vxFormatImagePatchAddress2d(src_base, x-3, y-3, &src_addr),
...
vx_status NonMaxSuppressionKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_FAILURE;
    if (num == 3)
    {
...
vx_status FAST9OutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 2) /* output image */
    {
...
vx_status IntegralWindowOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (index == 2) /* output image */
    {
...
vx_cle_tile_info IntegralWindowTilingValidator(vx_node node, vx_uint32 index)
{
    ...
    if (index == 0)
    {
        ...
        vxQueryParameter(param1, VX_PARAMETER_ATTRIBUTE_REF, &scalar, sizeof(scalar));
...
vx_status IntegralWindowKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_FAILURE;
    if (num == 3)
    {
...
status |= vxAccessScalarValue(winSize_param, &winSize);
        ...
        if (borders.mode == VX_BORDER_MODE_UNDEFINED)
        {
            ...
            for (y = 0; (y < (src_addr.dim_y - winSize)) && (status == VX_SUCCESS); y++)
            {
                for (x = 0; x < (src_addr.dim_x - winSize); x++)
                {
                    vx_uint32 *srcA = vxFormatImagePatchAddress2d(src_base, x+winSize, y+winSize, &src_addr);
                    vx_uint32 *srcB = vxFormatImagePatchAddress2d(src_base, x+1, y+1, &src_addr);
                    vx_uint32 *srcC = vxFormatImagePatchAddress2d(src_base, x+1, y+winSize, &src_addr);
                    vx_uint32 *srcD = vxFormatImagePatchAddress2d(src_base, x+winSize, y+1, &src_addr);
...
vx_status DisparityOutputValidator(vx_node node, vx_uint32 index, vx_meta_format_t *ptr)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
...
vx_status DisparityKernel(vx_node node, vx_reference *parameters, vx_uint32 num)
{
    vx_status status = VX_ERROR_INVALID_PARAMETERS;
    if (num == 6)
    {
        ...
        vx_uint32 x, y;
        void *in = NULL, *prev_min = NULL, *prev_out = NULL, *new_min = NULL, *out = NULL;
        vx_imagepatch_addressing_t addr_input, addr_prevMin, addr_prevOutput, addr_newMin, addr_output;
        vx_rectangle rect;
        vx_fourcc format = 0;
        vx_uint8 level = 0;
        ...
        status |= vxAccessImagePatch(prevOutput, rect, 0, &addr_prevOutput, &prev_out);
        ...
        for (y = 0; y < addr_input.dim_y; y+=addr_input.step_y)
        {
            for (x = 0; x < addr_input.dim_x; x+=addr_input.step_x)
            {
                ...
                vx_uint32 *prev_minp = vxFormatImagePatchAddress2d(new_min, x, y, &addr_prevMin);
                vx_uint32 *new_minp = vxFormatImagePatchAddress2d(new_min, x, y, &addr_newMin);
                vx_uint8  *prev_outp = vxFormatImagePatchAddress2d(prev_out, x, y, &addr_prevOutput);
        ...
        status |= vxCommitImagePatch(input, 0, 0, &addr_input, in); // don't write back into the input
        status |= vxCommitImagePatch(prevMin, 0, 0, &addr_prevMin, prev_min); // don't write back into the input
        status |= vxCommitImagePatch(prevOutput, 0, 0, &addr_prevOutput, prev_out); // don't write back into the input
        vxReleaseRectangle(&rect);
    }
...
vx_status vxPublishKernels(vx_context context)
{
    ...
    {
#ifdef CLE_ACCEL
    vx_cle_tile_info tile = (vx_cle_tile_info) {0, 0, {0, 0}, {0, 0}, CLE_STATE_NONE};
    vx_cle_tile_info tileInput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION} ;
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                            "it.unibo.retina.complementary",
                            VX_KERNEL_UNIBO_COMPLEMENTARY,
                            ComplementaryKernel,
                            2,
                            ComplementaryInputValidator,
                            ComplementaryOutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    {
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 0, VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileInput);
#else
        status = vxAddParameterToKernel(kernel, 0, VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED);
#endif
        if (status != VX_SUCCESS) goto exit;
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 1, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileOutput);
#else
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                            "it.unibo.retina.normalize",
                            VX_KERNEL_UNIBO_NORMALIZE,
                            NormalizeKernel,
                            4,
                            NormalizeInputValidator,
    ...
    if (kernel)
    {
        ...
        if (status != VX_SUCCESS) goto exit;
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 3, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileOutput);
#else
    ...
    {
#ifdef CLE_ACCEL
    vx_cle_tile_info tile = (vx_cle_tile_info) {25*sizeof(vx_float32), 0, {0, 0}, {0, 0}, CLE_STATE_SCALAR};
    vx_cle_tile_info tileInput = (vx_cle_tile_info) {1, 1, {2, 2}, {2, 2}, CLE_TRANSIENT_ALLOCATION} ;
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                            "it.unibo.retina.gaussian5x5",
                            VX_KERNEL_UNIBO_GAUSSIAN5X5,
                            Gaussian5x5Kernel,
                            3,
                            Gaussian5x5InputValidator,
                            Gaussian5x5OutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    if (kernel)
    {
        ...
        if (status != VX_SUCCESS) goto exit;
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 2, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileOutput);
#else
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.threshold_to_zero",
                            VX_KERNEL_UNIBO_THRESHOLD_TO_ZERO,
                            ThresholdToZeroKernel,
                            3,
                            ThresholdToZeroInputValidator,
                            ThresholdToZeroOutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.add_scalar",
                            VX_KERNEL_UNIBO_ADD_SCALAR,
                            AddScalarKernel,
                            4,
                            AddScalarInputValidator,
                            AddScalarOutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.divide",
                            VX_KERNEL_UNIBO_DIVIDE,
                            DivideKernel,
                            3,
                            DivideInputValidator,
                            DivideOutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    if (kernel)
    {
        ...
        if (status != VX_SUCCESS) goto exit;
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 1, VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileInput);
#else
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.sigmoid",
                            VX_KERNEL_UNIBO_SIGMOID,
                            SigmoidKernel,
                            7,
                            SigmoidInputValidator,
                            SigmoidOutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    if (kernel)
    {
        ...
        if (status != VX_SUCCESS) goto exit;
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 6, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileOutput);
#else
        status = vxAddParameterToKernel(kernel, 6, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED);
#endif
        if (status != VX_SUCCESS) goto exit;
        status = vxFinalizeKernel(kernel);
        if (status != VX_SUCCESS) goto exit;
    ...
    {
#ifdef CLE_ACCEL
    vx_cle_tile_info tile = (vx_cle_tile_info) {NB_CORES*sizeof(vx_float32)+NB_CORES*sizeof(vx_int32), 0, {0, 0}, {0, 0}, CLE_STATE_SCALAR};
    vx_cle_tile_info tileInput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION} ;
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.mean",
                            VX_KERNEL_UNIBO_MEAN,
                            MeanKernel,
                            2,
                            MeanInputValidator,
                            MeanOutputValidator,
...
#endif
        if (status != VX_SUCCESS) goto exit;
        status = vxAddParameterToKernel(kernel, 1, VX_OUTPUT, VX_TYPE_SCALAR, VX_PARAMETER_STATE_REQUIRED);
        if (status != VX_SUCCESS) goto exit;
        status = vxFinalizeKernel(kernel);
        if (status != VX_SUCCESS) goto exit;
    }
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.stddev",
                            VX_KERNEL_UNIBO_STDDEV,
                            StdDevKernel,
                            3,
                            StdDevInputValidator,
                            StdDevOutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    if (kernel)
    {
        ...
        status = vxAddParameterToKernel(kernel, 1, VX_INPUT, VX_TYPE_SCALAR, VX_PARAMETER_STATE_REQUIRED);
        if (status != VX_SUCCESS) goto exit;
        status = vxAddParameterToKernel(kernel, 2, VX_OUTPUT, VX_TYPE_SCALAR, VX_PARAMETER_STATE_REQUIRED);
        if (status != VX_SUCCESS) goto exit;
        status = vxFinalizeKernel(kernel);
        if (status != VX_SUCCESS) goto exit;
    }
...
#ifdef CLE_ACCEL
    vx_cle_tile_info tile = (vx_cle_tile_info) {0, 0, {0, 0}, {0, 0}, CLE_STATE_NONE};
    vx_cle_tile_info tileInput = (vx_cle_tile_info) {1, 1, {1, 1}, {1, 1}, CLE_TRANSIENT_ALLOCATION} ;
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.max3x3",
                            VX_KERNEL_UNIBO_MAX3x3,
                            Max3x3Kernel,
                            2,
                            Max3x3InputValidator,
                            Max3x3OutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
...
#ifdef CLE_ACCEL
    vx_cle_tile_info tile = (vx_cle_tile_info) {0, 0, {0, 0}, {0, 0}, CLE_STATE_NONE};
    vx_cle_tile_info tileInput = (vx_cle_tile_info) {1, 1, {3, 3}, {3, 3}, CLE_TRANSIENT_ALLOCATION} ;
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                           "it.unibo.retina.max7x7",
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                            "it.unibo.retina.fast9",
                            VX_KERNEL_UNIBO_FAST9,
                            FAST9Kernel,
                            3,
                            FAST9InputValidator,
                            FAST9OutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
...
#ifdef CLE_ACCEL
    vx_cle_tile_info tile = (vx_cle_tile_info) {0, 0, {0, 0}, {0, 0}, CLE_STATE_NONE};
    vx_cle_tile_info tileInput = (vx_cle_tile_info) {0, 0, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION} ;
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                            "it.unibo.retina.integral_window",
                            VX_KERNEL_UNIBO_INTEGRALWINDOW,
                            IntegralWindowKernel,
                            3,
                            IntegralWindowInputValidator,
                            IntegralWindowOutputValidator,
                            Initialize,
                            Deinitialize,
                            IntegralWindowTilingValidator,
                            tile,
                            "integral_window");
#else
    kernel = vxAddKernel(context,
    ...
    vx_cle_tile_info tileOutput = (vx_cle_tile_info) {1, 1, {0, 0}, {0, 0}, CLE_TRANSIENT_ALLOCATION};
    kernel = vxAddKernelCLE(context,
                            "it.unibo.retina.disparity",
                            VX_KERNEL_UNIBO_DISPARITY,
                            DisparityKernel,
                            6,
                            DisparityInputValidator,
                            DisparityOutputValidator,
                            Initialize,
                            Deinitialize,
                            NULL,
                            tile,
    ...
    if (kernel)
    {
        ...
        if (status != VX_SUCCESS) goto exit;
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 2, VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileInput);
#else
        status = vxAddParameterToKernel(kernel, 2, VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED);
#endif
        if (status != VX_SUCCESS) goto exit;
...
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 4, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileOutput);
#else
        status = vxAddParameterToKernel(kernel, 4, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED);
#endif
        if (status != VX_SUCCESS) goto exit;
#ifdef CLE_ACCEL
        status = vxAddParameterToKernelCLE(kernel, 5, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED, tileOutput);
#else
        status = vxAddParameterToKernel(kernel, 5, VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED);
#endif
        if (status != VX_SUCCESS) goto exit;
