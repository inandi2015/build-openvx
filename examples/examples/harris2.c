...
int main(int argc, char *argv[])
{
    ...
    if (context)
    {
        ...
        vx_scalar strength_thresh = vxCreateScalar(context, VX_TYPE_FLOAT32, &_strength_thresh);
        vx_scalar min_distance = vxCreateScalar(context, VX_TYPE_FLOAT32, &_min_distance);
        vx_scalar sensitivity = vxCreateScalar(context, VX_TYPE_FLOAT32, &_sensitivity);
	...
	vx_int32 gradient_size = 3; //taha 
	vx_int32 block_size = 3; //taha
        
	if (graph)
        {
            // Nodes
            vx_node nodes[] = {
              vxHarrisCornersNode(graph, images[0], strength_thresh, 
